...
uint8_t Study_Cmd_Repeat=0,STOP_CMD_Repeat=0;//连续按两下添码
volatile FLAG Flag;
//不同功能支持的遥控器命
...
uint8_t judge_remote_control(uint32_t roll_code)
{
	...
	{
		if (Capture_Command.Identify_data == 0x5AA55AA5) // 预埋遥控器
		{
			return TRUE;
		}
		for (i = 0; i < Flash_Data.Remote_Control_Num; i++) // 查找是否有存储的遥控器ID
...
void Record_PulseWidth(uint32_t g_tau0_ch7_width,CAPTURE_COMMAND *Capture_Command)
{
	...
	if(real_tau0_ch7_width>_800US_MAX_TIME)//判定出错
	   ...
	   default: break;
			...
			static uint16_t real_tau0_ch3_width = 0;
	if ((HEAD_HIGH_MIN_TIME <= g_tau0_ch7_width) && (g_tau0_ch7_width <= HEAD_HIGH_MAX_TIME))
	{
		real_tau0_ch3_width = 0;
		Capture_Command->Capture_Pulse_Wide_value[0] = g_tau0_ch7_width;
		Capture_Command->Capture_Pulse_Wide_Sequence = 1;
		Capture_Command->Capture_State = CAPTURE_FIXED_CODE;
		return;
	...
	case CAPTURE_FIXED_CODE: // 固定码和引导高电平
	{
		if (!_433_DATA) // 本次读取的是高电平
		{
			...
			{
				Flag.Need_433_data_compensate = 1;
				real_tau0_ch3_width += g_tau0_ch7_width;
				if (Capture_Command->Capture_Pulse_Wide_Sequence >= 2)
				{
					Capture_Command->Capture_Pulse_Wide_Sequence--;
				}
		...
		if ((real_tau0_ch3_width > _800US_MAX_TIME) && (Capture_Command->Capture_Pulse_Wide_Sequence > 1)) // 判定出错
		{
			Capture_Command->Capture_State = CAPTURE_HEAD_HIGH;
			Capture_Command->Capture_Pulse_Wide_Sequence = 0;
		}
			Capture_Command->Capture_Pulse_Wide_value[Capture_Command->Capture_Pulse_Wide_Sequence] = real_tau0_ch3_width;
			Capture_Command->Capture_Pulse_Wide_Sequence++;
			if (Capture_Command->Capture_Pulse_Wide_Sequence == FIXED_CODE_LEVELS) // 捕获81个电平时间
			{
				Capture_Command->Capture_State = CAPTURE_COMMANG_END;
				Capture_Command->Capture_433key_Judgetime = 0;
			}
...
{	
    ...
    static uint16_t Capture_433KEY_LONG_TICKS;//长按时间
            ...
            {			
                ...
                if(Capture_Command.Capture_433key_Cmd != Capture_Command.Command_data)
                {
                    ...
                    switch(Capture_Command.Capture_433key_Cmd)
                    {
                        ...
                        case MOTOR_UP_ALL:
                        {
                            Capture_433KEY_LONG_TICKS = 400;//长按3s	
                        }
                        break;
                        case STUDY_CMD:
                        {
                            Capture_433KEY_LONG_TICKS = 1300;//长按时间7s
                        }
                        break;
                        default:
                        {
                            Capture_433KEY_LONG_TICKS = 600;
                        }
                        break;
                    }
                }			
    ...
    if(cmd != NONE_PRESS)//
    {
        ...
        else if(cmd == PRESS_DOWN)
        {
            ...
            if(Capture_Command.Capture_433key_Cmd != MOTOR_STOP_CMD)
            {
                STOP_CMD_Repeat = 0;
						  	...
						  	Time.key_convergent=0;//清除按键间隔时间
            }
            ...
            switch(Capture_Command.Capture_433key_Cmd)
            {
                ...
                {	
										...
										{
												if(Err.err_bit.Encounter_Obstacle == 1 ||  Err_B.err_bit.Encounter_Obstacle== 1)
												 {
												  	STOP_CMD_Repeat++;
                ...
                {	
                    Flag.No_judge_electronic_limit = 0;
                    Flag.B_No_judge_electronic_limit = 0;
                    if((Device_State_Data.Motor_State_Byte == MOTOR_DOWN_STATE) && (Device_State_Data.B_Motor_State_Byte == MOTOR_DOWN_STATE))
                    {
                        Cmd_Motor_Stop();
                        Cmd_Motor_Stop_B();
                    }
                  ...
                  if(Device_State_Data.Control_open)
                    ...
                    match_data_del();
									...
									{	
										...
										{
											if(Err.err_bit.Encounter_Obstacle||Err_B.err_bit.Encounter_Obstacle)
											{
        ...
        /*执行遥控器长按*/	
        else if(cmd == LONG_RRESS_START)
        {
            switch(Capture_Command.Capture_433key_Cmd)
            {	
                case STUDY_CMD:
                {
                    if(( Flag.power_on_10stime_flag) && (!Flag.Add_Match_Code_flag))//上电10s后
                    {
                        Study_Cmd_Repeat = 0;
                        ...
                        else
                        {
                            Led.Light_Led_state = 1;//呼吸灯亮
											...
											Light_colour_temperature_gear=600;//重置后色温为最冷
                }
                break;
                case MOTOR_DOWN_A:   
                {
                ...
                case MOTOR_UP_ALL:
                {
                    if(Flag.B_ELE_LIM == 1 && Flag.ELE_LIM == 1)
                    {
                        Flag.Motor_all_up = 1;
                        Cmd_Motor_Up_Longpress();
                        Cmd_Motor_Up_Longpress_B();
